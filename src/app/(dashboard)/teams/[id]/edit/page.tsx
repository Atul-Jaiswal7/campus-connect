...
export default function EditTeamPage({ params }: EditTeamPageProps) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (!recRes.ok || !projRes.ok) throw new Error();
        ...
        if (recData.success && recData.data) {
          ...
          reset({
            ...
            hackathonName: rec.hackathonName ?? "",
            deadline: rec.deadline ? rec.deadline.split("T")[0] : undefined,
            projectId: rec.projectId ?? undefined,
            roles: rec.skills.map((s: { roleNeeded: string; skill: { name: string } }) => ({
  ...
  const onSubmit = async (values: TeamFormValues) => {
    ...
    try {
      ...
      toast({ title: "Recruitment updated successfully!" });
  ...
  if (loading) {
    return (
      <div className="flex min-h-[50vh] flex-col items-center justify-center gap-2">
        ...
        <p className="text-xs text-muted-foreground font-semibold">Loading recruitment data...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight">Edit Recruitment</h1>
          <p className="text-xs text-muted-foreground">Keep your team requirement postings up to date</p>
        </div>
      </div>
